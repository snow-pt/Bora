...
import { doc, updateDoc, deleteDoc, collection, onSnapshot, getDocs, arrayRemove } from 'firebase/firestore';
...
export default function EditTripScreen({ route, navigation }: any) {
  ...
  const handleRemoveBuddy = (email: string) => {
    Alert.alert("Remove Friend", `Are you sure you want to remove ${email}?`, [
      { text: "Cancel" },
      { text: "Remove", style: 'destructive', onPress: async () => {
          await updateDoc(doc(db, 'trips', tripData.id), { acceptedEmails: arrayRemove(email) });
          setBuddies(prev => prev.filter(b => b !== email));
      }}
    ]);
  };
  ...
  return (
    <SafeAreaView style={[styles.container, isDark && { backgroundColor: '#121212' }]}>
      ...
      <ScrollView style={styles.content} showsVerticalScrollIndicator={false}>
        ...
        {showPicker.visible && (
          <DateTimePicker value={showPicker.mode === 'start' ? startDate : endDate} mode="date" display="default" onChange={handleDateChange} />
        )}

        {/* --- EDIT BUDDIES --- */}
        <Text style={[styles.sectionTitle, isDark && { color: '#FFFFFF' }, { marginTop: spacing.xl }]}>Manage Buddies</Text>
